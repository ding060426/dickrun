...
python ./zipformer/decode.py \
    ...
    --encoder-unmasked-dim 192,192,256,320,256,192
    --blank-penalty 2
    --bpe-model data/lang_bbpe_10000/bbpe.model \
        --bpe-model data/lang_new/bpe.model \
...
from multi_dataset_test_domain import MultiDataset
...
def main():
    ...
    if not params.use_averaged_model:
        if params.iter > 0:
            ...
            load_checkpoint('/inspire/hdd/project/embodied-multimodality/chenxie-25019/chonghao/icefall/egs/multi_zh_en/ASR/zipformer/exp5-short/checkpoint-4000.pt', model)
        else:
            logging.info("Right Here!")
            # start = params.epoch - params.avg + 1
            filenames = ['./zipformer/exp5-short/checkpoint-132000.pt', './zipformer/exp5-short/checkpoint-128000.pt',
            './zipformer/exp5-short/checkpoint-124000.pt', './zipformer/exp5-short/checkpoint-120000.pt', './zipformer/exp5-short/checkpoint-116000.pt',]
            # filenames = []
    ...
    def remove_short_utt(c: Cut):
        ...
        if T <= 0:
            logging.warning(
                f"Excluding cut with ID: {c.id} from decoding, duration: {c.duration}"
            )
        return T > 0
