...
def ref_text_to_words(text: str) -> List[str]:
    return tokenize_by_CJK_char(str(text)).split()


def hyp_token_ids_to_words(
    token_ids: List[int],
    sp: spm.SentencePieceProcessor,
) -> List[str]:
    return sp.decode(token_ids).split()
        ...
        while len(decode_streams) >= params.num_decode_streams:
            ...
            for i in sorted(finished_streams, reverse=True):
                decode_results.append(
                    (
                        decode_streams[i].id,
                        ref_text_to_words(decode_streams[i].ground_truth),
                        hyp_token_ids_to_words(decode_streams[i].decoding_result(), sp),
                    )
                )
...
def main():
    ...
    def remove_short_utt(c: Cut):
        ...
        if T <= 0:
            logging.warning(
                f"Excluding cut with ID: {c.id} from decoding"
            )
        return T > 0
