...
模型输入以记录的 `full_text` 为事实来源，并支持工作会议、课程、讲座、访谈和演示等内容类型。JSON Schema 会直接附加到提示词；响应在渲染前还会把 `meeting_summary`、`main_topics`、`meeting_overviews` 等常见兼容字段归一化，随后执行结构校验。缺少外部 `jsonschema` 包时使用内置的必填字段与类型校验，不再静默跳过。
...
记录列表只读取会议级元数据；历史记录进入转写界面时使用 `include_audio=true` 完整返回分段 WAV Base64。前端据此重建 WaveSurfer 波形和播放按钮；CDN 不可用时使用占位波形和原生 `Audio` 播放器。
