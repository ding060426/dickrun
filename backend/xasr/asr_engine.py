...
class XASREngine:
    ...
    def process_file(
        ...
        if self.asr_optimizer:
            data, opt_report = self.asr_optimizer.prepare_audio(data, sr, denoise_enabled=denoise_enabled)
    ...
    def get_meeting_hotwords(self) -> List[str]:
        return list(self._meeting_hotwords or self._hotwords)


    def get_asr_optimizer_report(self) -> dict:
        if self._last_optimizer_report is not None:
            return self._last_optimizer_report
        if self.asr_optimizer and getattr(self.asr_optimizer, "last_report", None):
            return self.asr_optimizer.last_report.to_dict()
