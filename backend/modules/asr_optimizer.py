...
class ASROptimizer:
    ...
    def prepare_audio(self, audio: np.ndarray, sr: int, denoise_enabled: Optional[bool] = None) -> Tuple[np.ndarray, ASROptimizerReport]:
        """
        ...
        # 4. 可选 RNNoise 降噪；不可用或失败时自动旁路
        if denoise_enabled is None:
            rnnoise_mode = os.environ.get("DITING_RNNOISE_ENABLED", os.environ.get("RNNOISE_ENABLED", "auto")).lower()
            should_denoise = rnnoise_mode not in {"0", "false", "off", "no"}
        else:
            should_denoise = bool(denoise_enabled)

        if RNNoiseFilter is not None and should_denoise:
            try:
                if self._rnnoise is None or not self._rnnoise.status().get("enabled"):
                    self._rnnoise = RNNoiseFilter(sample_rate=sr, enabled=True)
                denoised, denoise_report = self._rnnoise.process_array(data, sr)
                report.denoise = denoise_report
                if denoise_report.get("applied"):
                    data = denoised
                    report.audio_actions.append("RNNoise 降噪已启用")
                elif denoise_report.get("available"):
                    report.audio_actions.append("RNNoise 已跳过，保留原始音频")
                else:
                    report.audio_actions.append("RNNoise 不可用，已旁路")
            except Exception as e:
                report.denoise = {"enabled": True, "available": False, "applied": False, "error": str(e)}
                report.audio_actions.append("RNNoise 处理失败，已回退原始音频")
        else:
            report.denoise = {"enabled": bool(should_denoise), "available": False, "applied": False, "reason": "disabled" if not should_denoise else "module_unavailable"}
