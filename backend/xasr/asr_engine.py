...
# ===========================================================================
# VAD: Silero neural VAD + energy fallback + max_duration protection
# ===========================================================================

# Try to use sherpa-onnx silero VAD (neural network, much more accurate)
_SILERO_VAD_MODEL = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models", "silero_vad.onnx")
_silero_vad = None
...
def _get_silero_vad():
    ...
    global _silero_vad, _silero_vad_loaded
    if _silero_vad_loaded:
        return _silero_vad
    _silero_vad_loaded = True
    if not os.path.isfile(_SILERO_VAD_MODEL):
        logger.info(f"Silero VAD model not found at {_SILERO_VAD_MODEL}, using energy VAD fallback")
        return None
    try:
        import sherpa_onnx
        cfg = sherpa_onnx.VadModelConfig()
        cfg.silero_vad.model = _SILERO_VAD_MODEL
        cfg.silero_vad.threshold = 0.5
        cfg.silero_vad.min_silence_duration = 0.5      # 500ms pause → sentence break
        cfg.silero_vad.min_speech_duration = 0.25      # 250ms min speech (filter noise)
        cfg.silero_vad.window_size = 512                # 32ms at 16kHz
        _silero_vad = sherpa_onnx.VoiceActivityDetector(cfg, buffer_size_in_seconds=120)
        logger.info(f"Silero VAD loaded (neural, threshold=0.5, min_silence=500ms)")
        return _silero_vad
    except Exception as e:
        logger.warning(f"Silero VAD load failed: {e}, using energy VAD fallback")
        return None


def _silero_vad_segment(
    audio: np.ndarray,
    sample_rate: int,
    ...
    """Use sherpa-onnx silero VAD for neural speech segmentation.
    
    Uses is_speech_detected() to track speech/non-speech transitions
    and build segments manually for full control.
    """
    vad = _get_silero_vad()
    if vad is None:
        return None  # Signal fallback to energy VAD
    ...
    window_size = 512  # silero requirement (32ms at 16kHz)

    # Feed audio window by window, track speech state
    segments = []
    in_speech = False
    speech_start = 0
    hop_samples = window_size  # 512 samples = 32ms

    for i in range(0, n_samples, hop_samples):
        ...
        while not vad.empty():
            seg = vad.front
            vad.pop()
            seg_start_sec = seg.start / sample_rate
            # seg.samples may be empty in some versions — estimate end from next silence
            if len(seg.samples) > 0:
                seg_end_sec = seg_start_sec + len(seg.samples) / sample_rate
            else:
                # No samples — use current position as end
                seg_end_sec = (i + window_size) / sample_rate
            segments.append((seg_start_sec, seg_end_sec))

        # Also use is_speech_detected for segments that VoiceActivityDetector misses
        if hasattr(vad, 'is_speech_detected'):
            is_speech_now = vad.is_speech_detected()
            if is_speech_now and not in_speech:
                in_speech = True
                speech_start = i / sample_rate
            elif not is_speech_now and in_speech:
                in_speech = False
                speech_end = i / sample_rate
                segments.append((speech_start, speech_end))

    # Flush and collect remaining
    vad.flush()
    while not vad.empty():
        seg = vad.front
        vad.pop()
        ...
        if len(seg.samples) > 0:
            ...
            seg_end_sec = total_duration
        segments.append((seg_start_sec, seg_end_sec))

    # Add the last segment if still in speech
    if in_speech:
        segments.append((speech_start, total_duration))

    # If no segments found, return the whole audio
    if not segments:
        logger.warning(f"Silero VAD: no speech detected in {total_duration:.1f}s, returning whole audio")
    ...
    # Merge overlapping/adjacent segments (within 0.5s)
    merged = []
    ...
    # Apply max_duration cap: split long segments
    final = []
    for seg in merged:
        dur = seg[1] - seg[0]
        if dur > max_duration:
    ...
    logger.info(f"Silero VAD: detected {len(final)} speech segments in {total_duration:.1f}s audio")
    return final
...
def _energy_vad(
    ...
    min_silence_frames: int = 50,          # ~500ms gap → sentence break (raised from 30)
    pre_padding_ms: float = 200,           # reduced from 300ms
    post_padding_ms: float = 200,          # reduced from 300ms
    min_segment_duration: float = 0.5,     # filter very short fragments
    max_duration: float = 30.0,            # NEW: force-split segments > 30s
) -> List[tuple]:
    """
    Detect speech segments using energy-based VAD (fallback when silero unavailable).
    ...
    # NEW: max_duration cap — force-split segments > 30s
    if max_duration > 0 and merged:
        capped = []
        for seg in merged:
            dur = seg[1] - seg[0]
            if dur > max_duration:
                ...
                for k in range(n_sub):
                    ...
                    sub_end = min(seg[1], seg[0] + (k + 1) * sub_dur)
                    if sub_end - sub_start >= min_segment_duration:
                        capped.append((sub_start, sub_end))
            else:
                capped.append(seg)
        merged = capped
...
class XASREngine:
    ...
    def process_file(
        ...
        # 2. VAD segmentation (FireRedVAD → Silero → Energy fallback)
        ...
        try:
            from modules.vad_manager import segment_audio as vad_segment_audio
            segments, vad_type = vad_segment_audio(
                data, sr,
                max_duration=30.0,
            ...
            logger.info(f"VAD: using {vad_type}, {len(segments)} segments")
        except ImportError:
            # Fallback to internal VAD if vad_manager not available
            segments = _silero_vad_segment(
                data, sr,
                max_duration=30.0,
                pre_padding_ms=200,
                post_padding_ms=200,
    ...
    def _build_result(
        ...
        # 1. Token-level confidence (three-layer: token_prob + acoustic + text)
        token_info = {}
        try:
            token_info = self.asr.get_token_info()
        except Exception:
            pass

        ys_probs = token_info.get("ys_probs", [])
        tokens_list = token_info.get("tokens", [])

        try:
            from modules.audio_processor import compute_token_confidence
            conf_data = compute_token_confidence(
                ys_probs=ys_probs,
                tokens=tokens_list,
                text=raw_text,
                snr_db=self._current_snr,
                rt60=self._current_rt60,
                hotwords=[],
            )
            asr_confidence = conf_data["confidence"]
            quality_label = conf_data["label"]
            quality = conf_data["acoustic_quality"]
            uncertain_tokens = conf_data.get("uncertain_tokens", [])
        except ImportError:
            # Fallback to old method if audio_processor not available
        ...
        # 5. Uncertain spans — now using real token-level data
        uncertain_spans = []
        if uncertain_tokens:
            # Convert uncertain tokens to span format
        ...
        elif self.enable_uncertainty and self._current_snr < 12:
            # Fallback: SNR-based uncertain spans for very low SNR
            words = display_text.split()
            for i, word in enumerate(words):
                if len(word) <= 2 and self._current_snr < 8:
                    uncertain_spans.append({
                        'start': i, 'end': i + 1, 'text': word,
                        'confidence': max(0.3, self._current_snr / 20),
                        'candidates': [word],
                    })
