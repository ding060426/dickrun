...
- **历史音频复现**：分段 WAV 与转写结果一同持久化；载入历史记录时恢复真实波形和播放按钮，WaveSurfer 不可用时保留占位波形并使用浏览器原生音频播放。
...
摘要以会议记录的 `full_text` 为事实来源，兼容工作会议、课程、讲座、访谈和产品演示。系统会把 JSON Schema 直接加入模型提示，并将常见兼容字段归一化到会悟结构；即使网关未安装额外校验库，也不会再静默接受缺字段的响应。
