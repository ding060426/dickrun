...
def get_backend_python():
    """Return a Python executable with backend dependencies installed."""
    env_python = os.environ.get("DITING_PYTHON")
    if env_python and Path(env_python).exists():
        return Path(env_python)

    py312 = Path.home() / ".conda" / "envs" / "py312" / "python.exe"
    if py312.exists():
        return py312
...
def start_backend():
    ...
    print(f"[DiTing] Backend Python: {backend_python}")
    ...
    proc = subprocess.Popen(
        [str(backend_python), "-m", "uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8765"],
        env=env,
    )
